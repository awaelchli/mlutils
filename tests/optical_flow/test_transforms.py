...
from mlutils.optical_flow import (
    warp,
    normalize,
    scale,
    resize,
...
def test_horizontal_warp():
    ...
    assert torch.all(warped == expected)
...
def test_scale():
    ...
    expected_x = 2 * flow_x
    expected_y = 2 * flow_y
    assert torch.all(scaled[:, 0] == expected_x)
    assert torch.all(scaled[:, 1] == expected_y)
...
def test_resize():
    ...
    assert torch.all(resized == expected)
